...
- **Animation support.** `magick` is optional and only needed for
  ...
  The chunk is cached so missing the package merely skips the demo.
...
- **Step 1 - Seed placement.** Select or draw a grid of starting seeds
  ...
  (rectangular, diamond, hexagonal, or random layouts) or crafted
  interactively with
  [`snic_grid_manual()`](https://rolfsimoes.github.io/snic/reference/snic_grid_manual.md).
- **Step 2 - Segmentation.** Run
  [`snic()`](https://rolfsimoes.github.io/snic/reference/snic.md) with
  the chosen seeds to grow superpixels and inspect the result with
  [`snic_plot()`](https://rolfsimoes.github.io/snic/reference/snic_plot.md)
  or the animated helper
  [`snic_animation()`](https://rolfsimoes.github.io/snic/reference/snic_animation.md).
...
returning a two-column (`r`, `c`) matrix ready for
...
Use
[`snic_count_seeds()`](https://rolfsimoes.github.io/snic/reference/snic_grid.md)
to forecast how many superpixels a spacing will produce before running
the algorithm.
...
interactively.
[`snic_grid_manual()`](https://rolfsimoes.github.io/snic/reference/snic_grid_manual.md)
opens a plotting device where you can add, move, or remove seeds
on-the-fly and then feed the result straight into
[`snic()`](https://rolfsimoes.github.io/snic/reference/snic.md):
...
Once seeds are defined, pass them to
[`snic()`](https://rolfsimoes.github.io/snic/reference/snic.md) together
with the imagery and a `compactness` factor. The result is a labeled
raster that can be visualized alongside the seeds for validation.
...
[`snic_animation()`](https://rolfsimoes.github.io/snic/reference/snic_animation.md)
replays the seeding process, adding one seed per frame, re-running
[`snic()`](https://rolfsimoes.github.io/snic/reference/snic.md), and
composing the frames into a GIF. Cache the chunk so the animation is
generated only once.
...
subset.](reference/figures/segmentation-animation.gif)
