---
title: "SNIC on Non-Earth-Observation Imagery"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{SNIC on Non-Earth-Observation Imagery}
...
SNIC works with any stack of numerical bands shaped as an array. This vignette shows how to read everyday images with a couple of popular packages and feed them into `snic()` without using `terra`. Each section runs only when the relevant reader package is installed; otherwise a short message is printed.

## Using `magick`

```{r magick-example, fig.width = 8, fig.height = 4.5}
...
```{r png-example, fig.width = 8, fig.height = 4.5}
...
```{r jpeg-example, fig.width = 8, fig.height = 4.5}
...
The `magick`, `png`, and `jpeg` packages cover a wide range of image formats, making SNIC practical for computer vision experiments, document analysis, or microscopy without leaving the R ecosystem.
