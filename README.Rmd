...
- Offers multiple seeding strategies (`snic_grid_rect()`, `snic_grid_diamond()`, `snic_grid_hexagon()`, `snic_grid_random()`) and interactive placement via `snic_grid_manual()`
...
- **Step 1 – Seed placement.** Select or draw a grid of starting seeds that guide where the superpixels will grow. Grids can be generated automatically (`snic_*_grid()`) or crafted interactively with `snic_grid_manual()`.
...
- `snic_grid_diamond()` – staggered rows produce a diagonal pattern that better respects gradients.
- `snic_grid_hexagon()` – hexagonal tiling for more isotropic superpixels.
- `snic_grid_random()` – jittered seeds when structure is irregular or prior knowledge is limited.
...
if (requireNamespace("magick", quietly = TRUE)) {
  set.seed(123)
  animation_seeds <- snic_grid_random(s2_demo, spacing = 20L, padding = 0L)
