...
- Offers multiple seeding strategies (`rect_grid()`, `diamond_grid()`,
  `hexagonal_grid()`, `random_grid()`) and interactive placement via
  `manual_grid()`
...
Sentinel-2 subset. The chunk is marked `eval = FALSE` to avoid heavy
computation when knitting.
...
spacing <- 8L
seeds <- rect_grid(s2_small, spacing = spacing, padding = 0L)
...
All seed generators share the same arguments and return a two-column
matrix (`r`, `c`) ready for `snic()`. Use `count_seeds()` to estimate
the expected number of superpixels for a given spacing before running
the segmentation.

``` r
spacing <- c(20L, 20L)

rect <- rect_grid(s2_small, spacing = spacing)
hex  <- hexagonal_grid(s2_small, spacing = spacing)
rand <- random_grid(s2_small, spacing = spacing)

count_seeds(s2_small, spacing = spacing)
#> [1] 70
```

When automatic strategies are not enough, `manual_grid()` launches an
interactive session (requires an interactive R session with plotting
enabled) that lets you refine or extend an existing seed set.

## Scripted demos

The repository provides a ready-to-run script demonstrating a full
segmentation workflow for the Rondônia Sentinel-2 stack:

- `inst/scripts/run_snic_rondonia.R`: command-line script that loads the
  packaged data, runs SNIC, and visualises the result.
